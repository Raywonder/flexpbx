...
<!--
    ...
    Changes:
    - Migrated from endpoint-based pattern (/api/trunks/delete) to query params (?path=delete&id={id})
    ...
    <script>
        ...
        function deleteTrunk(trunkId) {
            if (confirm('Are you sure you want to delete this trunk? This action cannot be undone.')) {
                // Send delete request to new comprehensive API
                fetch(`/api/trunks.php?path=delete&id=${trunkId}`, {
                    method: 'POST',
        ...
        function testTrunk(trunkId) {
            ...
            // Send test request to new comprehensive API
            fetch(`/api/trunks.php?path=test&id=${trunkId}`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ trunk_id: trunkId })
            }).then(response => response.json())
        ...
        function saveTrunk() {
            ...
            const trunkId = config.trunkName; // or appropriate ID field
            const apiPath = isUpdate ? `update&id=${trunkId}` : 'create';

            fetch(`/api/trunks.php?path=${apiPath}`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify(config)
            }).then(response => response.json())
        ...
        // Update real-time logs every 5 seconds using new comprehensive API
