...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && $token_valid) {
    ...
    if (empty($new_password) || empty($confirm_password)) {
        ...
        if (!file_exists($admin_file)) {
            ...
            // Update SIP password in pjsip.conf if admin has an extension
            $pjsip_updated = false;
            if (isset($admin_data['extension']) && !empty($admin_data['extension'])) {
                $pjsip_updated = updatePjsipPassword($admin_data['extension'], $new_password);
            }
