...
app.commandLine.appendSwitch('enable-features', 'AudioWorklet,AudioServiceOutOfProcess');
app.commandLine.appendSwitch('disable-background-throttling');
app.commandLine.appendSwitch('disable-renderer-backgrounding');
app.commandLine.appendSwitch('disable-backgrounding-occluded-windows');

// Enable accessibility features for VoiceOver
...
const VLCAudioService = require('./services/VLCAudioService');
...
class FlexPhoneMain {
    ...
    async initialize() {
        try {
            ...
            // Will be initialized when window is ready
    ...
    createMainWindow() {
        ...
        this.mainWindow = new BrowserWindow({
            ...
            webPreferences: {
                ...
                // Accessibility features for VoiceOver
                accessibleTitle: 'FlexPhone SIP Client',
                accessibleDescription: 'Accessible SIP phone interface'
    ...
    setupIPCHandlers() {
        ...
        // VLC Audio Service handlers
        ipcMain.handle('vlc-audio-play-welcome-tones', async () => {
            return await this.vlcAudioService.playWelcomeTones();
        });

        ipcMain.handle('vlc-audio-play-ringtone', async (event, ringtoneType = 'default') => {
            return await this.vlcAudioService.playRingtone(ringtoneType);
        });

        ipcMain.handle('vlc-audio-stop-ringtone', async () => {
            return await this.vlcAudioService.stopRingtone();
        });

        ipcMain.handle('vlc-audio-set-volume', async (event, volume) => {
            this.vlcAudioService.setVolume(volume);
            return this.vlcAudioService.getVolume();
        });

        ipcMain.handle('vlc-audio-get-volume', async () => {
            return this.vlcAudioService.getVolume();
        });

        ipcMain.handle('vlc-audio-is-available', async () => {
            ...
            },
            {
                ...
                submenu: [
                    ...
                    },
                    {
                        ...
                        submenu: [
                            {
                                label: 'FlexPBX Support Portal',
                                click: () => {
                                    shell.openExternal('https://flexpbx.devinecreations.net/support/report-issue');
                                }
                            },
                            {
                                label: 'GitHub Issues',
                                click: () => {
                                    shell.openExternal('https://github.com/rawonder/flexphone/issues');
                                }
                            }
                        ]
                    },
