...
class SSHService {
    ...
    async initializeServices() {
        try {
            ...
            console.warn('SSH Key Service not available:', error.message);
        }
    }

    /**
     * Get SSH key service instance
     */
    getSSHKeyService() {
...
echo "Access your FlexPBX installation at: http${sslEnabled ? 's' : ''}://${domain}"`;
    ...
    /**
     * Connect using saved connection with SSH key support
     */
    async connectWithSavedConnection(connectionId) {
        if (!this.sshKeyService) {
            throw new Error('SSH Key Service not available');
        }

        try {
            const connections = await this.sshKeyService.loadConnections();
            ...
            if (connectionInfo.authMethod === 'key' && connectionInfo.keyPath) {
                const privateKey = await fs.readFile(connectionInfo.keyPath, 'utf8');
                config.privateKey = privateKey;
            }

            const result = await this.connect(connectionId, config);

            if (result.success) {
                // Update last connected time
                await this.sshKeyService.updateConnectionLastUsed(connectionId);
            }
     ...
     */
    async testConnectionWithKey(server, keyPath) {
        try {
            ...
            return {
                success: false,
                error: `Failed to test key connection: ${error.message}`
            };
        }
    }

    /**
     * Setup SSH key authentication for server
     */
    async setupSSHKeyAuth(server, options = {}) {
        if (!this.sshKeyService) {
            throw new Error('SSH Key Service not available');
        }
        ...
        try {
            ...
            if (generateNew) {
                ...
                keyInfo = await this.sshKeyService.generateSSHKey({
                    keyName,
                    keyType,
                    comment: `FlexPBX-${server.host}-${new Date().toISOString()}`
                });
            } else {
                // Use existing key
                const keys = await this.sshKeyService.listSSHKeys();
                keyInfo = keys.find(k => k.keyName === keyName);

                if (!keyInfo) {
                    throw new Error(`SSH key not found: ${keyName}`);
                }
            }

            // Install public key on server if requested
            if (installKey) {
                await this.sshKeyService.installPublicKey(server, keyInfo.publicKey, 'password');
            }

            // Test the connection
            const testResult = await this.testConnectionWithKey(server, keyInfo.privateKeyPath);

            if (testResult.success) {
                // Save connection with SSH key
                const connectionInfo = await this.sshKeyService.saveConnection({
                    ...
                    tags: ['ssh-key', 'flexpbx']
                });

                // Add SSH config entry
                const configAlias = await this.sshKeyService.addSSHConfigEntry(server, keyInfo.privateKeyPath);
                ...
                return {
                    success: false,
                    error: 'SSH key authentication test failed',
                    testResult
                };
            }
        } catch (error) {
    ...
    async getSavedConnections() {
        ...
        return await this.sshKeyService.loadConnections();
    }
    ...
    async getConnectionStats() {
        ...
        Object.values(connections).forEach(conn => {
            ...
            if (conn.lastConnected && new Date(conn.lastConnected) > oneWeekAgo) {
                stats.recentlyUsed++;
            }
        });
    ...
    /**
     * Create secure connection with automatic key setup
     */
    async createSecureConnection(server, options = {}) {
        const {
            setupKeys = true,
            saveConnection = true,
            testConnection = true
        ...
        try {
            ...
            // Step 1: Test initial connection (usually with password)
            if (testConnection) {
                const initialTest = await this.testConnection({
                    host: server.host,
                    port: server.port || 22,
                    username: server.username,
                ...
                result.steps.push({
                    ...
                    message: initialTest.success ? 'Initial connection successful' : initialTest.error
                });
            ...
            if (setupKeys) {
                const keySetup = await this.setupSSHKeyAuth(server, {
                    generateNew: true,
                    installKey: true
                });
                ...
                result.steps.push({
                    step: 'ssh_key_setup',
                    success: keySetup.success,
                    message: keySetup.success ? 'SSH key authentication configured' : keySetup.error,
                    keyInfo: keySetup.keyInfo
                });

                if (keySetup.success) {
                    result.connectionInfo = keySetup.connectionInfo;
                    result.keyInfo = keySetup.keyInfo;
                }
            }

            // Step 3: Save connection if requested
            if (saveConnection && !result.connectionInfo) {
                const connectionInfo = await this.sshKeyService.saveConnection({
                    name: server.name || `${server.username}@${server.host}`,
                    host: server.host,
                    port: server.port || 22,
                    username: server.username,
                    authMethod: server.password ? 'password' : 'key',
                    tags: ['flexpbx']
                });
